...
## Keep-alive de Evolution API en Render

Render free tier duerme los servicios tras ~15 minutos de inactividad. Para evitar
que la Evolution API quede inactiva y falle el envio de WhatsApp, existe un workflow
n8n que la mantiene despierta.

**Workflow:** `Evolution API Keep-Alive` (ID: `PaNSIuiuHsIL96Mw`)
**Nodos:** Schedule Trigger (cada 10 minutos) → HTTP Request GET `https://qamiluna-evolution-api.onrender.com`
**Estado:** publicado y activo desde 2026-04-16.

Si en algun momento se detecta que Evolution vuelve a dormirse:

1. Verificar que el workflow esta activo en `https://n8n.qamilunastudio.com/workflow/PaNSIuiuHsIL96Mw`.
2. Revisar la pestana `Executions` del workflow para confirmar que corre cada 10 min.
3. Si el workflow esta pausado, volver a activarlo con el toggle de la esquina superior derecha.
4. Como solucion de emergencia inmediata, visitar `https://qamiluna-evolution-api.onrender.com`
   desde el navegador para despertar el servicio manualmente.
